...
<? } ?>
  ...
  <div class="comment content_top2 comment_<?=$comment['c_seq']?>">
    ...
    <button class="btn btn-outline-primary btn-sm c_modify" data-c_seq="<?=$comment['c_seq']?>" data-permission="<?=$comment['permission']?>">수정하기</button>
...
<? if (!isset($_GET['search_input'])) {?>
  <div class="text-center my-5"> 
    <? $pre = $bottom['pre']; $next = $bottom['next']; ?>
    <? if (!isset($next) && isset($pre)) {?>
    <a href="/board/get_content?b_seq=<?=$pre['b_seq']?>&list=<?=$list?>">이전 게시글 : <?=$pre['subject']?></a>
    <? } else if (!isset($pre) && isset($next)) {?>
    <a href="/board/get_content?b_seq=<?=$next['b_seq']?>&list=<?=$list?>">다음 게시글 : <?=$next['subject']?></a>
    <? } else {?>
    <a href="/board/get_content?b_seq=<?=$next['b_seq']?>&list=<?=$list?>">다음 게시글 : <?=$next['subject']?></a><br>
    <a href="/board/get_content?b_seq=<?=$pre['b_seq']?>&list=<?=$list?>">이전 게시글 : <?=$pre['subject']?></a>
    <? } ?>
  </div>
  <!-- 검색 결과기 있을때 네비게이터 -->
<? } else { ?>
  <div class="text-center my-5"> 
    <? $pre = $bottom['pre']; $next = $bottom['next']; ?>
    <? if (!isset($next) && isset($pre)) {?>
    <a href="/board/get_content?b_seq=<?=$pre['b_seq']?>&list=<?=$list?>&search_by=<?=$_GET['search_by']?>&search_input=<?=$_GET['search_input']?>">이전 게시글 : <?=$pre['subject']?></a>
    <? } else if (!isset($pre) && isset($next)) {?>
    <a href="/board/get_content?b_seq=<?=$next['b_seq']?>&list=<?=$list?>&search_by=<?=$_GET['search_by']?>&search_input=<?=$_GET['search_input']?>">다음 게시글 : <?=$next['subject']?></a>
    <? } else {?>
    <a href="/board/get_content?b_seq=<?=$next['b_seq']?>&list=<?=$list?>&search_by=<?=$_GET['search_by']?>&search_input=<?=$_GET['search_input']?>">다음 게시글 : <?=$next['subject']?></a><br>
    <a href="/board/get_content?b_seq=<?=$pre['b_seq']?>&list=<?=$list?>&search_by=<?=$_GET['search_by']?>&search_input=<?=$_GET['search_input']?>">이전 게시글 : <?=$pre['subject']?></a>
    <? } ?>
  </div>
<? } ?>